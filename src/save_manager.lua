...
---@param saveKey ConstructSaveKeys
---@param dataDuration DataDuration
---@param data table
local function addDefaultData(saveKey, dataDuration, data)
	if not SaveManager.Utility.IsConstructSaveKey(saveKey) then return end
	local keyToType = {
...
---@param dataType ConstructSaveKeys
---@param data table
...
local function getListIndex(lastIndex)
	return tostring(lastIndex and game:GetLevel():GetLastRoomDesc().ListIndex or game:GetLevel():GetCurrentRoomDesc().ListIndex)
end
...
local function populatePickupData(pickup)
	...
	if pickupData then
		...
		SaveManager.Utility.SendDebugMessage("Successfully populated pickup data from floor-saved room data for", SaveManager.Utility.GetSaveIndex(pickup))
		local pickupIndex = SaveManager.Utility.GetPickupIndex(pickup)
		if game:GetRoom():GetType() == RoomType.ROOM_BOSS then
